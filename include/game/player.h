...
typedef struct {
    ...
    int invl, suit;
    char d;
    unsigned char ku, kd, kl, kr, kf, kj, kwl, kwr, kp;
    int frag, ammo, shel, rock, cell, fuel, kills, secrets;
    unsigned char fire, cwpn, csnd;
    unsigned char amul;
    unsigned short wpns;
    char wpn;
    unsigned char f;
    unsigned char drawst;
    unsigned char color;
    int id;
    unsigned char keys;
    char lives;
...
extern unsigned char p_immortal;
extern player_t pl1, pl2;

/* PL_drawst: кэш для оптимизации перерисовки UI */
extern int ui_prev_life, ui_prev_armor, ui_prev_ammo, ui_prev_air;
extern char ui_prev_wpn;
extern unsigned char ui_prev_keys, ui_prev_drawst;
