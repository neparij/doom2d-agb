...
static pcm_ref_t snd[14];
static void *spr[49*2];
static char sprd[49*2];
BN_DATA_EWRAM static weapon_t wp[MAXWPN];

static void w_throw(int,int,int,int,int,int,int,int);
	...
	switch(wp[i].t) {
	  case REVF:
		if(Z_getobjpos(wp[i].target,&o))
		  w_throw(i,wp[i].o.x,wp[i].o.y-2,o.x+o.xv+o.vx,o.y+o.yv+o.vy,2,5,12);
	  case ROCKET:
  ...
  for(i=0;i<MAXWPN;++i) {
    ...
    switch(wp[i].t) {
      ...
      case ROCKET:
		if((d=wp[i].s)<2) {
		  d=(wp[i].o.xv>0)?1:0;
		  x=bn::abs(wp[i].o.xv);y=wp[i].o.yv;s=0;
		  if(y<0) {if(-y>=x) s=30;}
    ...
    if(s>=0 && R_obj_in_view(wp[i].o.x,wp[i].o.y,wp[i].o.r,wp[i].o.h))
      Z_drawspr(wp[i].o.x,wp[i].o.y,spr[s*2+d],sprd[s*2+d]);
  }
}

BN_CODE_IWRAM void WP_gun(int x,int y,int xd,int yd,int o,int v) {
  unsigned int d,m,inv_d;
  int sx,sy,lx,ly;
  unsigned int xe,ye,s,step;
  unsigned char f;
  ...
  inv_d=(1u<<16)/d;
  hit_xv=((unsigned)(xd*10)*inv_d>>16)*sx;
  hit_yv=((unsigned)(yd*10)*inv_d>>16)*sy;
  xe=ye=0;
  lx=x;ly=y;
  for(;;) {
	...
	if(((m=bmap[y>>5][x>>5])&f)) {
	  if(m&BM_WALL) if(fld[y>>3][x>>3]==1 || fld[y>>3][x>>3]==2) {
		for(x=lx,y=ly,xe=ye=0;fld[y>>3][x>>3]!=1 && fld[y>>3][x>>3]!=2;) {
		  lx=x;ly=y;
		  xe+=xd; step=(xe*inv_d)>>16; x+=step*sx; xe-=step*d;
		  ye+=yd; step=(ye*inv_d)>>16; y+=step*sy; ye-=step*d;
		}
		DOT_spark(lx,ly,sx*10,sy*10,1);
		break;
	  }
	  if(m&(BM_MONSTER|BM_PLR1|BM_PLR2)) if(Z_gunhit(x,y,o,sx*v,sy*v,lx,ly,sx,sy)) break;
	  lx=x;ly=y;
	  xe+=xd<<3; step=(xe*inv_d)>>16; x+=step*sx; xe-=step*d;
	  ye+=yd<<3; step=(ye*inv_d)>>16; y+=step*sy; ye-=step*d;
	}else{
	  if(sx==0) m=0;
	  ...
	  if((s<m && s!=0) || m==0) m=s;
	  lx=x;ly=y;
	  step=(xd*m+xe)*inv_d>>16; x+=step*sx; xe=(xd*m+xe)-step*d;
	  step=(yd*m+ye)*inv_d>>16; y+=step*sy; ye=(yd*m+ye)-step*d;
	}
  }
}
...
static void w_throw(int i,int x,int y,int xd,int yd,int r,int h,int s) {
  int m;
...
void WP_ognemet(int x,int y,int xd,int yd,int xv,int yv,int o) {
  ...
  m=bn::abs(xd-x);if(!m) m=bn::abs(yd-y);
  // SMK_flame(x,y,xv,yv,2,2,(xd-x)*3000/m,(yd-y)*3000/m,1,o); // TODO: add flame effect
}
