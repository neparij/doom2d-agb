...
BN_DATA_EWRAM static pcm_ref_t snd[4];
BN_DATA_EWRAM static void *spr[58];
BN_DATA_EWRAM static char sprd[58];
static int tsndtm,rsndtm;
BN_DATA_EWRAM static item_t it[MAXITEM];
...
int IT_load(const unsigned char* p, unsigned size) {
  int m, i, j;
  old_thing_t t;
  ...
  switch (blk.t) {
    case MB_THING:
      BN_LOG("IT_load: MB_THING");
      for (i = 0; left >= sizeof(t); ++i) {
      ...
      if (!g_dm) {
        if (j == -1) BN_ERROR("IT_load: player 1 thing not found");
        dm_pos[0].x = it[j].o.x;
        dm_pos[0].y = it[j].o.y;
      ...
      if (!g_dm && _2pl) {
        if (j == -1) BN_ERROR("IT_load: player 2 thing not found");
        dm_pos[1].x = it[j].o.x;
        dm_pos[1].y = it[j].o.y;
      ...
      if (g_dm && j < 2) BN_ERROR("IT_load: fewer than 2 DM start points");
      if (g_dm) {
        dm_pnum = j;
...
void IT_draw(void) {
  ...
  for(i=0;i<MAXITEM;++i) {
	...
	if(s>=0 && R_obj_in_view(it[i].o.x,it[i].o.y,it[i].o.r,it[i].o.h))
