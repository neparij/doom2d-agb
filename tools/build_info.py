...
def get_current_build_info() -> Tuple[str, str]:
    ...
    __current_build_version_re = re.search(r'#define\s+D2DGBA_BUILD_VERSION\s+"([^"]+)"', __build_info_text)
    __current_build_commit_re = re.search(r'#define\s+D2DGBA_BUILD_COMMIT\s+"([^"]+)"', __build_info_text)
...
def update_build_info_file(current_build_info, new_version, new_commit):
    ...
    if new_version != current_build_version:
        ...
        build_info_text = re.sub(
            r'#define\s+D2DGBA_BUILD_VERSION\s+".*?"',
            fr'#define D2DGBA_BUILD_VERSION "{new_version}"',
            build_info_text
        )
    ...
    if new_commit != current_build_commit:
        ...
        build_info_text = re.sub(
            r'#define\s+D2DGBA_BUILD_COMMIT\s+".*?"',
            fr'#define D2DGBA_BUILD_COMMIT "{new_commit}"',
            build_info_text
        )
    ...
    build_info_text = re.sub(
        r'#define\s+D2DGBA_BUILD_DATE\s+".*?"',
        fr'#define D2DGBA_BUILD_DATE "{build_time}"',
        build_info_text
    )
    ...
    build_info_text = re.sub(
        r'#define\s+D2DGBA_BUILD_HOST_OS\s+".*?"',
        fr'#define D2DGBA_BUILD_HOST_OS "{build_env[0]}"',
        build_info_text
    )
    build_info_text = re.sub(
        r'#define\s+D2DGBA_BUILD_HOST_ENV\s+".*?"',
        fr'#define D2DGBA_BUILD_HOST_ENV "{build_env[1]}"',
        build_info_text
    )
    build_info_text = re.sub(
        r'#define\s+D2DGBA_BUILD_COMPILER\s+".*?"',
        fr'#define D2DGBA_BUILD_COMPILER "{build_env[2]}"',
        build_info_text
    )
